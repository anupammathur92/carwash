...
class Customerapi extends CI_Controller
{
	...
	private function addCustomer()
	{
		...
		if($this->input->post("customer_name"))
			$inputParams["customer_name"] = $this->filter($this->input->post("customer_name"));

		if($this->input->post("email"))
			$inputParams["email"] = $this->filter($this->input->post("email"));

		if($this->input->post("mobile_number"))
			$inputParams["mobile_number"] = $this->filter($this->input->post("mobile_number"));

		if($this->input->post("password"))
			$inputParams["password"] = $this->filter($this->input->post("password"));

		if(!empty($inputParams))
			$this->Customerapi_model->addCustomer($inputParams);
		else
			$this->something_went_wrong();
	}
